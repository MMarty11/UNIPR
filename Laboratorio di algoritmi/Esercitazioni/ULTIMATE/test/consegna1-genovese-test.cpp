...
void timSort(int arr[], int start, int end, int RUN)
{
     
    // Sort individual subarrays of size RUN
    for (int i = start; i < end; i+=RUN)
...
void sort(int *A, int n, int _max, int _min, int test)
{
...
int main(int argc, char **argv)
{
  ...
  int _max; /// valore massimo nell'array di input (counting sort)
  int _min; /// valore massimo nell'array di input (counting sort)
  ...
  for (test = 0; test < ntests; test++)
  {
    ...
    for (i = 0; i < n; i++)
    {
      ...
      // trovo il valore massimo contenuto in A
      /*if (k_max < A[i])
        k_max = A[i];
  ...
  delete [] A;  /// dealloco l'array di supporto per i conteggi (dipende da k = massimo nell'array in uso)
  delete [] B_output;
  delete [] C_counter;
  delete [] L;
  delete [] R;
