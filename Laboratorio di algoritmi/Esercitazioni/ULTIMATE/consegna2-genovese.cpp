...
// compilazione: g++ -xc++ lezione11-1-tree-dot-eulero.c 
//
...
/// stampa i valori dell'albero nell'ordine dell'omonima funzione
...
/// calcola l'altezza/profondità di un albero binario
int depth_tree(node_t* n){
  // inizializzo le profondità del sottoalbero destro e sinistro
  int depth_L=0, depth_R=0;

  if(n->R != NULL)
    depth_R = depth_tree(n->R);

  if(n->L != NULL)
    depth_L = depth_tree(n->L);
...
node_t* build_euler(int pre){  // n1 = preordine
  int n_L,n_R;  //prima, a metà e dopo la chiamata
  int temp; //lettura centrale
  bool leftIsNull = true;
  node_t* root;  //puntatore al nodo
  root = node_new(pre);
  ...
  if (pre!=n_L) {
    //printf("PADRE: %d --> FIGLIO SINISTRO: %d", pre, n_L);  /// DEBUG
    root->L = build_euler(n_L);
    leftIsNull = false;
  }
  //else {}  // in questo caso il valore è lo stesso, quindi non c'è il sottalbero sinistro


  input_visit >> n_R;

  if (pre!=n_R){
    //printf("PADRE: %d --> FIGLIO DESTRO: %d", pre, n_L);  /// DEBUG
    root->R = build_euler(n_R);
    if(leftIsNull)
      input_visit >> temp;
  }
  else {
...
*/
  ...
  */
  ...
  /// la stampa deve mostrare l'albero corretto
  tree_print_graph(root);

  /// scambio tutti i figli destri e sinistri
  node_t* n_flip = flip(root);

  /// la stampa deve mostrare l'albero scambiato
  tree_print_graph(n_flip);

  /// calcolo l'altezza/profondità dell'albero
  int depth = depth_tree(root);


  // la visita di eulero deve coincidere con il file da cui si e' creato l'albero
  //EulerOrder(root);
