...
void bst_insert(int x,node_t* node){
  ...
  if (x < node->val){
    ...
    if (node->L != NULL){
      printf("proseguo nel sottoalbero L\n");
      bst_insert(x,node->L);
    }
    else{
      printf("non c'e' il sottoalbero L -> aggiungo!\n");
      tree_insert_child_L(node,x);
    }
    return;
  }
  ...
  if (x > node->val){
    ...
    if (node->R != NULL){
      printf("proseguo nel sottoalbero R\n");
      bst_insert(x,node->R);
    }
    else{
      printf("non c'e' il sottoalbero R -> aggiungo!\n");
      tree_insert_child_R(node,x);
    }
    return;
  }
...
int altezza(node_t* nodo){
  ...
  int l=-1,r=-1;   /// valori di default se trovo foglia
  if (nodo->L != NULL)
    l= altezza(nodo->L);
  if (nodo->R != NULL)
    r= altezza(nodo->R);
...
int main(int argc, char **argv) {
  ...
  tree_insert_child_R(root->L->L,8);
