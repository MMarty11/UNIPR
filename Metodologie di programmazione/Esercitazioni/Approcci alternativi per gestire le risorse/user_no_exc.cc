...
bool codice_utente() {
  ...
  if (r1 == nullptr) {
	...
	/* exception safe livello massimo: 
	     - non devo rilasciare nulla perché non ho ancora acquisito nulla
		 - devo essere neutrale rispetto alle eccezioni, devo segnalare agli altri che si è verificato un problema (return true)
	*/
    return true;
  }

  // acquisita r1 con successo: devo ricordarmi di rilasciarla

  if (usa_risorsa(r1)) {
    // errore durante l'uso: rilascio r1
    restituisci_risorsa(r1);
    return true; // avverto che qualcosa è andato storto
  }
  ...
  // acquisita r2 con successo: devo ricordarmi di rilasciare r2 e r1
